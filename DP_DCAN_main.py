...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='DP-DCAN: Differentially Private Deep Contrastive Autoencoder Network for Single-cell Clustering',
    ...
    parser.add_argument('--beta1', default=0.4, type=float)
    parser.add_argument('--beta2', default=0.4, type=float)
    ...
    parser.add_argument('--steps_dp', default=900, type=int)
    ...
    parser.add_argument('--save_dir', default='checkpoints/DP-SCC/',
    ...
    model = DP_DCAN(input_dim=adata.n_vars, z_dim=args.z_dim, encodeLayer=[256, 64], decodeLayer=[64, 256],
    ...
    if args.ae_weights is None:
        model.pretrain_autoencoder(X=adata.X, X_raw=adata.raw.X, size_factor=adata.obs.size_factors,
                                   ...
                                   alpha=args.alpha, c1_dropout=args.c1_dropout, c2_dropout=args.c2_dropout,
                                   l2_norm_clip=args.l2_norm_clip, noise_multiplier=args.noise_multiplier, wd=wandb)
    else:
    ...
    if args.n_clusters > 0:
        y_pred, _, _, _, _ = model.fit(X=adata.X, X_raw=adata.raw.X, size_factor=adata.obs.size_factors,
                                       ...
                                       l2_norm_clip=args.l2_norm_clip, ae_save=True,
                                       noise_multiplier=args.noise_multiplier, steps_dp=args.steps_dp, wd=wandb,
